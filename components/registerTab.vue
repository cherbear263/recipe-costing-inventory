<template>
  <form @submit.prevent="signUp" class="flex flex-col px-16 py-8 rounded-md shadow-lg max-w-screen-sm w-full bg-white">
    <div class=" justify-center gap-2 mb-4 text-custom-darkblue">
      ...
      <div class="flex-row ">
        <h1 class="block text-slate-700 font-bold mb-3 mt-10">Register</h1>
        ...
        <input type="password" name="password" v-model="password" placeholder="password"
          class="w-full py-2 rounded-md px-4 border-2 border-slate-200 focus:bg-slate-100 outline-blue-200 mt-3" />
        <input type="submit"
          class="bg-blue-500 px-4 py-2 mt-4 text-white font-semibold hover:bg-blue-400 cursor-pointer rounded-md w-full" />
